...
- **Auto-travel — *Settled (design) — deferred to M2 (build)*.** Tap a distant **remembered** tile
  ...
  - **It is one command, `travel(to)`**, resolving many turns inside `step()` — never a loop above
    ...
    stands for**: same states, same fuel, same creature turns. It is not allowed to be a mechanic.
  ...
  - **You stop for the living.** Travel stops after a step in which a creature you were not
    perceiving became perceived, a creature woke, or your HP went down; and before a step into an
    occupied tile, because **travel never attacks**. **Terrain never interrupts** — the narrowing is
    argued in the change log below. Contact is keyed to the creature, not the tile, and *losing*
    contact is not "new".
