...
- **Auto-travel — *Settled (design) — deferred to M2 (build)*.** Tap a distant **remembered** tile
  ...
  - **You stop for the living.** Travel stops after a step in which a creature you were not
    ...
    argued in the change log below. Contact is keyed to the creature, not the tile; *losing* contact
    is not "new"; and it is judged on the finished state of each step, which is the state a player
    tapping one step at a time would have been looking at.
