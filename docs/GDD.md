...
| `N things wake. You gather N ember.` | **alarm** | The only compound the line has (#107, below). The level is the winning cue's, and the wake won it — a hunter is still what is against you |
...
- **It never costs the alarm its glance.** The wake sentence is first and unchanged, so a player who
  ...
  red `C` *first*.
- **It fits on one line at the default text size, and the descend compound did not.**
  `status-line.tsx` measures 36 mono characters at 14px as ~313pt inside 362pt of row at 390 wide, so
  the row holds **41 characters**. The longest compound the game can produce is
  ...
  Something wakes.` is **43** and wraps at 100%. That is the whole difference between the two cases,
  and it is arithmetic rather than taste. **41 is a budget, not a fact about today's font:** if the
  ramp moves, this number is what gets re-derived.
