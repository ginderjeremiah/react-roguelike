 ...
 * creature-list computation, journal entries for #17 and #19 — not `game/fov/light.ts`, whose
 * `computeLitField` is alive and consumed). A retained predecessor state is also not free — it pins a whole `Floor`, its grid,
 ...
 * often enough to matter, and rarely enough that it is not noise — which is the point this sentence
 * was making, and it survives the correction. Per ADR-0013 and #127; do not "fix" it back to one in
 * five.
 *
