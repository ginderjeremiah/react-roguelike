...
- **The clearing turns are charged to a tank that is now a clock.** A never-flash line earns nothing
  under clause 1, so it burns 80 fuel at 1 a turn and is dead inside one floor — `STALKER` already
  spends **81 turns** on a floor (measured, corpus). Every turn spent walking to a sleeper before
  lighting anything is a turn of that budget, and today it is free because 0 fuel is not an ending.
  **Without clause 2 this answer is unavailable, which is the whole reason the two clauses are one
  ruling.**
...
reading of this paragraph would fire [#145](../../issues/145)'s second criterion.** ADR-0015's *waking
...
one of #145's stated criteria — which is the cleanest evidence available that they are one
subtraction:
...
- **Light-gating the drop alone** — #144's recommendation, and it is adopted as *half* of this ruling
  ...
  dying**, because 0 fuel is not an ending: the corpus reaches the stairs on 80 of 80 dry floors.
  #145's second criterion (*over ≥9 seeds the never-flash line must not reach floor 8 on every seed at
  full HP*) comes back red, and proposition (a) — *die of the dark* — is untouched, because there is
  nothing in a shuttered floor for a moving player to die of **except the dark itself**.
- **0-fuel lethality alone** — **#144 rejects this by name and ADR-0015 cites the rejection**, so the
  ...
  and the reason it gives is *"`STALKER` nets +6 a floor, so it does not bind"*. That reason is about
  a **flashing** style and it is correct. It does not reach the pair, and the arithmetic says why: a
  never-flash fighter banks 60-120 a floor against ~81 turns of crawl, so alone it is solvent and
  keeps out-earning `STALKER` — which fires #145's **third** criterion (invariant 4) rather than
  clearing it. It is the `CACHE_FUEL` trap from the other side: it would satisfy the criterion this
  project measures *(a never-flash line can die)* while leaving the wager exactly as one-sided as it
  is now.
- **Together** they clear all three: `HARVESTER` earns nothing and cannot out-earn `STALKER`; a
  never-flash line has 80 fuel and no income and dies inside a floor; and the flash is the only thing
  that pays, so a playtest that is glad it flashed is the ordinary case rather than the hoped-for one.
...
which is true. The decisive objection is that **it cannot clear #145's second criterion at all**: the
