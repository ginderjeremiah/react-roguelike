...
> over 2000 seeds through `openRun`: **223, or 11.2%** (11.25% at 20 000). Per depth: floor 1 **11.2%**, then 14.7 / 17.9 / **20.6%**, flat from floor 4 down (the `min` caps spawn at 6, so floors 4-8 are structurally identical and measure bit-identically) — so the 20% is the deep-arrival rate. **A run start *is* a floor-1 arrival**, so that is one
